...
export const confirmCompanyControllerGet = async (req: Request, res: Response): Promise<void> => {
    ...
    if (!companyProfile) {
        logger.error(createLogMessage(req, confirmCompanyControllerGet.name, `error: company profile was not found in session`));
    ...
    if (!companyProfile.registeredOfficeAddress || Object.keys(companyProfile.registeredOfficeAddress).length === 0) {
        ...
        logger.error(createLogMessage(req, confirmCompanyControllerGet.name, `error: company ${companyProfile.companyNumber} does not have a registered office address`));
    }
    ...
    logger.info(createLogMessage(req, confirmCompanyControllerGet.name, "Rendering confirm company page"));
...
export const confirmCompanyControllerPost = async (req: Request, res: Response): Promise<void> => {
    ...
    if (!company) {
        logger.error(createLogMessage(req, confirmCompanyControllerPost.name, `error: company profile was not found in session`));
        throw new Error("company profile not found in session");
    }
    ...
    logger.info(createLogMessage(req, confirmCompanyControllerPost.name, `Redirecting to ${nextPageUrl}`));
